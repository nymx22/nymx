...
let hoverHint; // Reference to hover hint element
let isHoveringFirstPole = false;
let firstPoleClickable = true;

/* - - Setup - - */
function setup() {
  createCanvas(windowWidth, windowHeight);
  
  // Get hover hint element
...
function draw() {
  ...
  if (pixelationAmount > 0) {
    ...
    pixelBuffer.background(255); // White
    ...
    for (let i = 0; i < poles.length; i++) {
      // Add glow effect to first pole when hovering
      let shouldGlow = (i === 0 && isHoveringFirstPole);
      poles[i].display(shouldGlow);
    }
  }
  ...
  // Check hover on first pole
  checkFirstPoleHover();
}

/* - - Check Hover on First Pole - - */
function checkFirstPoleHover() {
  if (poles.length === 0 || !hoverHint) return;
  
  let firstPole = poles[0];
  let wasHovering = isHoveringFirstPole;
  isHoveringFirstPole = firstPole.containsPoint(mouseX, mouseY);
  
  if (isHoveringFirstPole) {
    // Show hint text
    if (!wasHovering) {
      hoverHint.classList.add('visible');
    }
    // Position hint text above the pole, centered horizontally
    let hintX = firstPole.x;
    let hintY = firstPole.y - firstPole.shaftHeight * firstPole.scale - 50;
    hoverHint.style.left = hintX + 'px';
    hoverHint.style.top = hintY + 'px';
    hoverHint.style.transform = 'translateX(-50%)'; // Center horizontally
  } else {
    // Hide hint text
    if (wasHovering) {
...
function mouseClicked() {
  if (poles.length === 0 || !firstPoleClickable) return false;
  
  let firstPole = poles[0];
  if (firstPole.containsPoint(mouseX, mouseY)) {
    // Navigate to glitch engine page
    window.location.href = 'glitchengine.html';
